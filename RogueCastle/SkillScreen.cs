...
namespace RogueCastle {
    public class SkillScreen : Screen {
        ...
        public override void LoadContent() {
            ...
            m_descriptionDivider.ScaleX = 250f / (float)m_descriptionDivider.Width;
            ...
            m_bg.Scale = new Vector2(1320f / (float)m_bg.Width, 1320f / (float)m_bg.Width);
            m_bg.ForceDraw = true;
            ...
            m_cloud1.Position = new Vector2((float)CDGMath.RandomInt(0, 1520), (float)CDGMath.RandomInt(0, 360));
            m_cloud2.Position = new Vector2((float)CDGMath.RandomInt(0, 1520), (float)CDGMath.RandomInt(0, 360));
            m_cloud3.Position = new Vector2((float)CDGMath.RandomInt(0, 1520), (float)CDGMath.RandomInt(0, 360));
            m_cloud4.Position = new Vector2((float)CDGMath.RandomInt(0, 1520), (float)CDGMath.RandomInt(0, 360));
            m_cloud5.Position = new Vector2((float)CDGMath.RandomInt(0, 1520), (float)CDGMath.RandomInt(0, 360));
            ...
            m_titleText.X = (float)m_titleText.Width / 2f + 20f;
            m_titleText.Y = 64.8f;
            ...
            if (!childAt.Visible) {
                ...
                switch (manorIndex) {
                    ...
                    case 28:
                        num = 0.5f;
                        childAt.Opacity = 0f;
                        Tween.To(childAt, num, new Easing(Tween.EaseNone), new[] {
                            "Opacity",
                            "1"
                        });
                        goto IL_A26;
                    ...
                    case 5:
                        ...
                        Tween.By(childAt, num, new Easing(Quad.EaseOut), new[] {
                            "X",
                    ...
                    case 4:
                        ...
                        goto IL_A26;
                    case 7:
                        pos.X = (float)childAt.AbsBounds.Right - (childAt.Width * 2) * 0.25f;
                        childAt.Opacity = 1f;
                        num = 3f;
                        childAt.Y += (float)(childAt.Height * 2);
                        SoundManager.PlaySound(new[] {
                    ...
                    case 31:
                        ...
                        num = 1f;
                        Tween.To(childAt, num, new Easing(Bounce.EaseOut), new[] {
                            "ScaleX",
                            "1",
                            "ScaleY",
                            "1"
                ...
                childAt.Opacity = 1f;
                childAt.Y -= 720f;
                Tween.By(childAt, num, new Easing(Quad.EaseIn), new[] {
                    "Y",
                    "720"
                });
        ...
        public void SetSkillIconVisible(SkillObj skill) {
            ...
            foreach (SkillObj current in SkillSystem.GetAllConnectingTraits(skill)) {
                if (!current.Visible) {
                    current.Visible = true;
                    current.Opacity = 0f;
                    Tween.To(current, 0.2f, new Easing(Linear.EaseNone), new[] {
                        "Opacity",
                        "1"
                    });
                    num += 0.2f;
        ...
        public void EndShake() {
            if (m_shookLeft)
                m_shakeObj.X += (float)m_shakeAmount;
            m_shakeObj = null;
            m_shakeTimer = 0f;
        }
        ...
        public override void Update(GameTime gameTime) {
            if (!m_cameraTweening && m_selectedTraitIndex != new Vector2(7f, 1f) && base.Camera.Y != 360f) {
                m_cameraTweening = true;
                Tween.To(base.Camera, 0.5f, new Easing(Quad.EaseOut), new[] {
                    "Y",
                    360f.ToString()
                });
            ...
            if (m_cloud1.Bounds.Right < -100)
                m_cloud1.Position = new Vector2((float)CDGMath.RandomInt(1420, 1520), (float)CDGMath.RandomInt(0, 360));
            if (m_cloud2.Bounds.Right < -100)
                m_cloud2.Position = new Vector2((float)CDGMath.RandomInt(1420, 1520), (float)CDGMath.RandomInt(0, 360));
            if (m_cloud3.Bounds.Right < -100)
                m_cloud3.Position = new Vector2((float)CDGMath.RandomInt(1420, 1520), (float)CDGMath.RandomInt(0, 360));
            if (m_cloud4.Bounds.Right < -100)
                m_cloud4.Position = new Vector2((float)CDGMath.RandomInt(1420, 1520), (float)CDGMath.RandomInt(0, 360));
            if (m_cloud5.Bounds.Right < -100)
                m_cloud5.Position = new Vector2((float)CDGMath.RandomInt(1420, 1520), (float)CDGMath.RandomInt(0, 360));
            m_cloud1.X -= 20f * num;
            m_cloud2.X -= 16f * num;
            m_cloud3.X -= 15f * num;
            m_cloud4.X -= 5f * num;
            ...
            if (!m_cameraTweening && !m_lockControls) {
                ...
                if (SkillSystem.IconsVisible) {
                    ...
                    if (selectedTraitIndex != m_selectedTraitIndex) {
                        ...
                        Tween.To(skill3, 0.1f, new Easing(Back.EaseOutLarge), new[] {
                            "ScaleX",
                            "1",
                            "ScaleY",
                            "1"
        ...
        public void UpdateDescriptionPlate(SkillObj trait) {
            ...
            m_inputDescription.Y = (float)(m_skillDescription.Bounds.Bottom + 10);
            ...
            if (num > -1f) {
                ...
                if (m_inputDescription.Text != " " && m_inputDescription.Text != "")
                    m_skillCurrent.Y = (float)(m_inputDescription.Bounds.Bottom + 40);
                else
                    m_skillCurrent.Y = (float)(m_skillDescription.Bounds.Bottom + 40);
        ...
        public override void Draw(GameTime gameTime) {
            m_cloud1.Y = (m_cloud2.Y = (m_cloud3.Y = (m_cloud4.Y = (m_cloud5.Y = base.Camera.TopLeftCorner.Y * 0.2f))));
            m_bg.Y = base.Camera.TopLeftCorner.Y * 0.2f;
            ...
            m_cloud1.Draw(base.Camera);
            m_cloud2.Draw(base.Camera);
            m_cloud3.Draw(base.Camera);
            m_cloud4.Draw(base.Camera);
            m_cloud5.Draw(base.Camera);
            ...
            m_toggleIconsText.Draw(base.Camera);
            m_confirmText.Draw(base.Camera);
            m_navigationText.Draw(base.Camera);
            m_playerMoney.Draw(base.Camera);
            base.Camera.GraphicsDevice.SamplerStates[0] = SamplerState.PointClamp;
            m_descriptionDivider.Draw(base.Camera);
            m_coinIcon.Draw(base.Camera);
            base.Camera.End();
            base.Draw(gameTime);
        }
        ...
        public void UpdateShake() {
            if (m_horizontalShake) {
                m_bg.X = (float)CDGMath.RandomPlusMinus() * (CDGMath.RandomFloat(0f, 1f) * m_screenShakeMagnitude);
                m_manor.X = (float)CDGMath.RandomPlusMinus() * (CDGMath.RandomFloat(0f, 1f) * m_screenShakeMagnitude);
            }
            if (m_verticalShake) {
                m_bg.Y = (float)CDGMath.RandomPlusMinus() * (CDGMath.RandomFloat(0f, 1f) * m_screenShakeMagnitude);
                m_manor.Y = (float)CDGMath.RandomPlusMinus() * (CDGMath.RandomFloat(0f, 1f) * m_screenShakeMagnitude);
            }
        }
