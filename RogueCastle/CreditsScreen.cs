...
namespace RogueCastle {
    public class CreditsScreen : Screen {
        ...
        public override void LoadContent() {
            ...
            m_bgOutside.Scale = new Vector2(1320f / (float)m_bgOutside.Width, 1320f / (float)m_bgOutside.Width);
            ...
            m_bg2.X -= (float)m_bg1.Width;
            m_bg3 = (m_bg2.Clone() as SpriteObj);
            m_bg3.X -= (float)m_bg2.Width;
            ...
            m_ground2.X -= (float)m_ground1.Width;
            m_ground3 = (m_ground2.Clone() as SpriteObj);
            m_ground3.X -= (float)m_ground2.Width;
            ...
            m_border2.X -= (float)m_border1.Width;
            m_border3 = (m_border2.Clone() as SpriteObj);
            m_border3.X -= (float)m_border2.Width;
            ...
            m_sideBorderLeft.Scale = new Vector2(900f / (float)m_sideBorderLeft.Width, 500f / (float)m_sideBorderLeft.Height);
            ...
            m_sideBorderTop.Scale = new Vector2(1320f / (float)m_sideBorderTop.Width, (240 / m_sideBorderTop.Height));
            ...
            m_sideBorderBottom.Scale = new Vector2(1340f / (float)m_sideBorderBottom.Width, 720f / (float)m_sideBorderBottom.Height);
        ...
        public override void ReinitializeRTs() {
            ...
            m_skyRenderTarget = new RenderTarget2D(base.Camera.GraphicsDevice, 1320, 720);
            m_backgroundRenderTarget = new RenderTarget2D(base.Camera.GraphicsDevice, 1320, 720);
        ...
        public override void OnEnter() {
            ...
            foreach (TextObj current in m_creditsTitleList) {
                Tween.By(current, 75f, new Easing(Tween.EaseNone), new[] {
                    "Y",
                    m_scrollDistance.ToString()
                });
            }
            foreach (TextObj current2 in m_creditsNameList) {
                Tween.By(current2, 75f, new Easing(Tween.EaseNone), new[] {
                    "Y",
                    m_scrollDistance.ToString()
                });
            }
            Tween.By(m_teddy, 75f, new Easing(Tween.EaseNone), new[] {
                "Y",
                m_scrollDistance.ToString()
            });
            Tween.By(m_kenny, 75f, new Easing(Tween.EaseNone), new[] {
                "Y",
                m_scrollDistance.ToString()
            });
            Tween.By(m_glauber, 75f, new Easing(Tween.EaseNone), new[] {
                "Y",
                m_scrollDistance.ToString()
            });
            Tween.By(m_gordon, 75f, new Easing(Tween.EaseNone), new[] {
                "Y",
                m_scrollDistance.ToString()
            });
            Tween.By(m_judson, 75f, new Easing(Tween.EaseNone), new[] {
                "Y",
                m_scrollDistance.ToString()
            });
            if (!IsEnding) {
        ...
        public void SetPlayerStyle(string animationType) {
            ...
            m_playerSprite.Y = 435f - ((float)m_playerSprite.Bounds.Bottom - m_playerSprite.Y);
        }
        ...
        public override void HandleInput() {
            if ((!IsEnding || (IsEnding && m_allowExit)) && (Game.GlobalInput.JustPressed(0) || Game.GlobalInput.JustPressed(1) || Game.GlobalInput.JustPressed(2) || Game.GlobalInput.JustPressed(3))) {
                if (m_displayingContinueText) {
                    ...
                    Tween.To(m_continueText, 0.5f, new Easing(Tween.EaseNone), new[] {
                        "Opacity",
                        "1"
                    });
        ...
        private void UpdateBackground(float elapsedTime) {
            int num = 200;
            m_bg1.X += (float)num * elapsedTime;
            m_bg2.X += (float)num * elapsedTime;
            m_bg3.X += (float)num * elapsedTime;
            if (m_bg1.X > 930f)
                m_bg1.X = m_bg3.X - (float)m_bg3.Width;
            if (m_bg2.X > 930f)
                m_bg2.X = m_bg1.X - (float)m_bg1.Width;
            if (m_bg3.X > 930f)
                m_bg3.X = m_bg2.X - (float)m_bg2.Width;
            m_ground1.X += (float)num * elapsedTime;
            m_ground2.X += (float)num * elapsedTime;
            m_ground3.X += (float)num * elapsedTime;
            if (m_ground1.X > 930f)
                m_ground1.X = m_ground3.X - (float)m_ground3.Width;
            if (m_ground2.X > 930f)
                m_ground2.X = m_ground1.X - (float)m_ground1.Width;
            if (m_ground3.X > 930f)
                m_ground3.X = m_ground2.X - (float)m_ground2.Width;
            m_border1.X += (float)num * elapsedTime;
            m_border2.X += (float)num * elapsedTime;
            m_border3.X += (float)num * elapsedTime;
            if (m_border1.X > 930f)
                m_border1.X = m_border3.X - (float)m_border3.Width;
            if (m_border2.X > 930f)
                m_border2.X = m_border1.X - (float)m_border1.Width;
            if (m_border3.X > 930f)
                m_border3.X = m_border2.X - (float)m_border2.Width;
            m_prop1.X += (float)num * elapsedTime;
            m_prop2.X += (float)num * elapsedTime;
            m_prop3.X += (float)num * elapsedTime;
            if (m_prop1.X > 930f)
                m_prop1.X -= (float)CDGMath.RandomInt(1000, 3000);
            if (m_prop2.X > 930f)
                m_prop2.X -= (float)CDGMath.RandomInt(1000, 3000);
            if (m_prop3.X > 930f)
                m_prop3.X -= (float)CDGMath.RandomInt(1000, 3000);
        }

        public void SwapBackground(string levelType) {
            Tween.By(m_sideBorderLeft, 0.5f, new Easing(Tween.EaseNone), new[] {
                "X",
                "200"
            });
            Tween.By(m_sideBorderRight, 0.5f, new Easing(Tween.EaseNone), new[] {
                "X",
                "-200"
            });
        ...
        public void PerformSwap(string levelType) {
                            ...
                            if (!(levelType == "Garden")) {
                                if (!(levelType == "Outside")) {
                                    if (levelType == "Manor") {
                                        ...
                                        Tween.By(m_manor, 3f, new Easing(Tween.EaseNone), new[] {
                                            "X",
                                            "300"
                                        });
                                        Tween.By(m_bgOutside, 3f, new Easing(Tween.EaseNone), new[] {
                                            "X",
                                            "300"
                                        });
                                        Tween.By(m_playerSprite, 3.5f, new Easing(Tween.EaseNone), new[] {
                                            "X",
                                            "-150"
                                        });
                                        Tween.AddEndHandlerToLastTween(this, "CreditsComplete", new object[0]);
                                        Tween.By(m_sideBorderTop, 2.5f, new Easing(Tween.EaseNone), new[] {
                                            "Y",
                                            "-500"
                                        });
                                    }
            ...
            Tween.By(m_sideBorderLeft, 3f, new Easing(Tween.EaseNone), new[] {
                "X",
                "-800"
            });
            Tween.By(m_sideBorderRight, 3f, new Easing(Tween.EaseNone), new[] {
                "X",
                "800"
            });
        }
        ...
        public void CreditsComplete() {
            ...
            Tween.To(m_thanksForPlayingText, 2f, new Easing(Tween.EaseNone), new[] {
                "Opacity",
                "1"
            });
            Tween.To(m_totalDeaths, 2f, new Easing(Tween.EaseNone), new[] {
                "delay",
                "0.2",
                "Opacity",
                "1"
            });
            Tween.To(m_totalPlayTime, 2f, new Easing(Tween.EaseNone), new[] {
                "delay",
                "0.4",
                "Opacity",
                "1"
        ...
        public void BringWife() {
            ...
            m_wifeSprite.PlayAnimation(true);
            Tween.By(m_wifeSprite, 3f, new Easing(Tween.EaseNone), new[] {
                "X",
                "600"
            });
        ...
        public void BringChild1() {
            ...
            m_childSprite1.PlayAnimation(true);
            Tween.By(m_childSprite1, 3f, new Easing(Tween.EaseNone), new[] {
                "X",
                "600"
            });
        ...
        public void BringChild2() {
            ...
            m_childSprite2.PlayAnimation(true);
            Tween.By(m_childSprite2, 2f, new Easing(Tween.EaseNone), new[] {
                "X",
                "600"
            });
