...
namespace RogueCastle {
    public class TextScreen : Screen {
        ...
        public override void OnEnter() {
            m_smoke1.Position = new Vector2((float)CDGMath.RandomInt(300, 1000), m_text.Y + (float)m_text.Height / 2f - 30f + (float)CDGMath.RandomInt(-100, 100));
            m_smoke2.Position = new Vector2((float)CDGMath.RandomInt(200, 700), m_text.Y + (float)m_text.Height / 2f - 30f + (float)CDGMath.RandomInt(-50, 50));
            m_smoke3.Position = new Vector2((float)CDGMath.RandomInt(300, 800), m_text.Y + (float)m_text.Height / 2f - 30f + (float)CDGMath.RandomInt(-100, 100));
            m_smoke1.Opacity = (m_smoke2.Opacity = (m_smoke3.Opacity = 0f));
            Tween.To(m_smoke1, m_fadeInSpeed, new Easing(Tween.EaseNone), new[] {
                "Opacity",
                "0.3"
            });
            Tween.To(m_smoke2, m_fadeInSpeed, new Easing(Tween.EaseNone), new[] {
                "Opacity",
                "0.2"
            });
            Tween.To(m_smoke3, m_fadeInSpeed, new Easing(Tween.EaseNone), new[] {
                "Opacity",
                "0.15"
            });
            BackBufferOpacity = 0f;
            m_text.Opacity = 0f;
            Tween.To(this, m_fadeInSpeed, new Easing(Tween.EaseNone), new[] {
                "BackBufferOpacity",
                m_backBufferOpacity.ToString()
            });
            Tween.To(m_text, m_fadeInSpeed, new Easing(Tween.EaseNone), new[] {
                "Opacity",
                "1"
            });
            if (m_typewriteText) {
                ...
                Tween.RunFunction(m_fadeInSpeed, m_text, "BeginTypeWriting", new object[] {
                    (float)m_text.Text.Length * 0.05f,
                    ""
                });
            }
            else
        ...
        public override void Draw(GameTime gametime) {
            ...
            m_smoke1.Draw(base.Camera);
            m_smoke2.Draw(base.Camera);
            m_smoke3.Draw(base.Camera);
