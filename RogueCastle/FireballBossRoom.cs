...
namespace RogueCastle {
    public class FireballBossRoom : BossRoomObj {
        ...
        public void DisplayOrb(int index) {
            Tween.To(m_fireList[index], 0.2f, new Easing(Quad.EaseOut), new[] {
                "Opacity",
                "1"
            });
        ...
        public void AbsorbOrb(int index) {
            ...
            Tween.To(m_fireList[index], 0.2f, new Easing(Quad.EaseIn), new[] {
                "X",
                m_boss.X.ToString(),
                "Y",
                m_boss.Y.ToString()
            });
            Tween.To(m_fireList[index], 0.1f, new Easing(Tween.EaseNone), new[] {
                "delay",
                "0.1",
                "Opacity",
                "0"
        ...
        public override void Update(GameTime gameTime) {
            if (m_boss.CurrentHealth <= 0 && base.ActiveEnemies > 1) {
                foreach (EnemyObj current in base.EnemyList) {
                    if (current is EnemyObj_BouncySpike)
                        current.Kill(false);
                }
