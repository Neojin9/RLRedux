...
namespace RogueCastle {
    public class CarnivalShoot1BonusRoom : BonusRoomObj {
        ...
        public override void LoadContent(GraphicsDevice graphics) {
            ...
            base.GameObjList.Add(m_targetIcons);
            base.GameObjList.Add(m_daggerIcons);
        ...
        public override void Initialize() {
            ...
            foreach (TerrainObj current2 in base.TerrainObjList) {
                if (current2.Name == "Floor") {
                    m_elf.Y = current2.Y - ((float)m_elf.Bounds.Bottom - m_elf.Y);
                    num = current2.Y;
                    break;
                }
            }
            ...
            if (!base.IsReversed) {
                ...
                m_rewardChest.Position = new Vector2(m_elf.X + 100f, num - (float)m_rewardChest.Height - 8f);
            }
            else
                m_rewardChest.Position = new Vector2(m_elf.X - 150f, num - (float)m_rewardChest.Height - 8f);
        ...
        public override void OnEnter() {
            ...
            if (!base.IsReversed) {
                ...
                m_daggerIcons.X -= (float)(400 + m_daggerIcons.Width);
            }
            else {
                m_targetIcons.Position = new Vector2((this.Bounds.Left + 150), (this.Bounds.Bottom - 40));
                m_daggerIcons.Position = m_targetIcons.Position;
                m_daggerIcons.X += (float)(m_targetIcons.Width + 400);
            }
            m_daggerIcons.Y -= 30f;
            ReflipPosters();
            base.OnEnter();
        ...
        public void BeginGame() {
            ...
            m_spokeToNPC = true;
            Player.AttachedLevel.CameraLockedToPlayer = false;
            Player.AttachedLevel.Camera.Y = (float)this.Bounds.Center.Y;
            if (!base.IsReversed) {
        ...
        public void EndGame() {
            if (!base.IsReversed) {
                Tween.To(Player.AttachedLevel.Camera, 1f, new Easing(Quad.EaseInOut), new[] {
                    "X",
                    (base.X + (float)Player.AttachedLevel.Camera.Width / 2f).ToString()
                });
            }
            else {
                Tween.To(Player.AttachedLevel.Camera, 1f, new Easing(Quad.EaseInOut), new[] {
        ...
        public override void Update(GameTime gameTime) {
            ...
            if (!base.IsReversed) {
                if (Player.X >= m_line.X - 150f)
                    Player.X = (float)((int)m_line.X - 150);
            }
            else if (Player.X < m_line.X + 150f)
                Player.X = m_line.X + 150f;
            if (!base.IsReversed) {
                if (m_isPlayingGame && Player.X < (float)Player.AttachedLevel.Camera.Bounds.Left)
                    Player.X = (float)Player.AttachedLevel.Camera.Bounds.Left;
                if (Player.X > (float)(this.Bounds.Right - 1320))
                    Player.X = (float)(this.Bounds.Right - 1320);
            }
            else {
                if (m_isPlayingGame && Player.X > (float)Player.AttachedLevel.Camera.Bounds.Right)
                    Player.X = (float)Player.AttachedLevel.Camera.Bounds.Right;
                if (Player.X < (float)(this.Bounds.Left + 1320))
                    Player.X = (float)(this.Bounds.Left + 1320);
            }
            if (m_currentTarget != null && !m_currentTarget.Broken) {
                float num = (float)gameTime.ElapsedGameTime.TotalSeconds;
                if (m_targetMovingUp && m_currentTarget.Bounds.Top > this.Bounds.Top + 80)
                    m_currentTarget.Y -= num * m_targetSpeed;
                else if (m_targetMovingUp) {
                    m_currentTarget.Y += num * m_targetSpeed;
                    m_targetMovingUp = false;
                }
                if (!m_targetMovingUp && m_currentTarget.Bounds.Bottom < this.Bounds.Bottom - 140)
                    m_currentTarget.Y += num * m_targetSpeed;
                else if (!m_targetMovingUp) {
                    m_currentTarget.Y -= num * m_targetSpeed;
            ...
            if (m_elf.IsTouching && !base.RoomCompleted && !m_spokeToNPC) {
        ...
        public void ActivateTarget() {
            ...
            if (!base.IsReversed) {
                Tween.By(m_currentTarget, 2f, new Easing(Quad.EaseOut), new[] {
                    "X",
