...
namespace RogueCastle {
    public class ProceduralLevelScreen : Screen {
        ...
        public override void LoadContent() {
            ...
            m_projectileIconPool = new ProjectileIconPool(200, m_projectileManager, base.ScreenManager as RCScreenManager);
            m_projectileIconPool.Initialize();
            m_textManager.Initialize();
            m_impactEffectPool.Initialize();
            m_physicsManager = (base.ScreenManager.Game as Game).PhysicsManager;
            m_physicsManager.SetGravity(0f, 1830f);
            m_projectileManager.Initialize();
            m_physicsManager.Initialize(base.ScreenManager.Camera);
            ...
            m_miniMapDisplay.InitializeAlphaMap(new Rectangle(1070, 50, 200, 100), base.Camera);
            ...
            m_blackBorder1.Scale = new Vector2(1340f / (float)m_blackBorder1.Width, (m_borderSize / m_blackBorder1.Height));
            m_blackBorder2 = new SpriteObj("Blank_Sprite");
            m_blackBorder2.TextureColor = Color.Black;
            m_blackBorder2.Scale = new Vector2(1340f / (float)m_blackBorder2.Width, (m_borderSize / m_blackBorder2.Height));
            ...
            m_objectivePlate.Scale = new Vector2(250f / (float)m_objectivePlate.GetChildAt(0).Width, 130f / (float)m_objectivePlate.GetChildAt(0).Height);
            ...
            m_sky.LoadContent(base.Camera);
            m_whiteBG = new SpriteObj("Blank_Sprite");
            m_whiteBG.Opacity = 0f;
            m_whiteBG.Scale = new Vector2(1320f / (float)m_whiteBG.Width, 720f / (float)m_whiteBG.Height);
        ...
        public override void ReinitializeRTs() {
            m_sky.ReinitializeRT(base.Camera);
            m_miniMapDisplay.InitializeAlphaMap(new Rectangle(1070, 50, 200, 100), base.Camera);
            InitializeRenderTargets();
            ...
            if (CurrentRoom == null || CurrentRoom.Name != "Start") {
                if (CurrentRoom.Name == "ChallengeBoss") {
                    ...
                    m_backgroundSprite.ChangeSprite("NeoBG_Sprite", base.ScreenManager.Camera);
                    m_backgroundSprite.Scale = new Vector2(2f, 2f);
                    m_foregroundSprite.Scale = Vector2.One;
                    m_foregroundSprite.ChangeSprite("NeoFG_Sprite", base.ScreenManager.Camera);
                    ...
                    switch (CurrentRoom.LevelType) {
                        case GameTypes.LevelType.CASTLE:
                            m_backgroundSprite.Scale = Vector2.One;
                            m_foregroundSprite.Scale = Vector2.One;
                            m_backgroundSprite.ChangeSprite("CastleBG1_Sprite", base.ScreenManager.Camera);
                            m_foregroundSprite.ChangeSprite("CastleFG1_Sprite", base.ScreenManager.Camera);
                            m_backgroundSprite.Scale = new Vector2(2f, 2f);
                            m_foregroundSprite.Scale = new Vector2(2f, 2f);
                            break;
                        case GameTypes.LevelType.GARDEN:
                            m_backgroundSprite.Scale = Vector2.One;
                            m_foregroundSprite.Scale = Vector2.One;
                            m_backgroundSprite.ChangeSprite("GardenBG_Sprite", base.ScreenManager.Camera);
                            m_foregroundSprite.ChangeSprite("GardenFG_Sprite", base.ScreenManager.Camera);
                            m_backgroundSprite.Scale = new Vector2(2f, 2f);
                            m_foregroundSprite.Scale = new Vector2(2f, 2f);
                            break;
                        case GameTypes.LevelType.DUNGEON:
                            m_backgroundSprite.Scale = Vector2.One;
                            m_foregroundSprite.Scale = Vector2.One;
                            m_backgroundSprite.ChangeSprite("DungeonBG1_Sprite", base.ScreenManager.Camera);
                            m_foregroundSprite.ChangeSprite("DungeonFG1_Sprite", base.ScreenManager.Camera);
                            m_backgroundSprite.Scale = new Vector2(2f, 2f);
                            m_foregroundSprite.Scale = new Vector2(2f, 2f);
                            break;
                        case GameTypes.LevelType.TOWER:
                            m_backgroundSprite.Scale = Vector2.One;
                            m_foregroundSprite.Scale = Vector2.One;
                            m_backgroundSprite.ChangeSprite("TowerBG2_Sprite", base.ScreenManager.Camera);
                            m_foregroundSprite.ChangeSprite("TowerFG2_Sprite", base.ScreenManager.Camera);
                            m_backgroundSprite.Scale = new Vector2(2f, 2f);
                            m_foregroundSprite.Scale = new Vector2(2f, 2f);
                            break;
                    }
        ...
        public void InitializeAllRooms(bool loadContent) {
            ...
            m_roomBWRenderTarget = new RenderTarget2D(base.Camera.GraphicsDevice, 1320, 720);
            foreach (RoomObj current in RoomList) {
                int num2 = 0;
        ...
        public void AddRooms(List<RoomObj> roomsToAdd) {
            foreach (RoomObj current in roomsToAdd) {
                ...
                if (current.X < (float)m_leftMostBorder)
                    m_leftMostBorder = (int)current.X;
                if (current.X + (float)current.Width > m_rightMostBorder)
                    m_rightMostBorder = (int)current.X + current.Width;
                if (current.Y < (float)m_topMostBorder)
                    m_topMostBorder = (int)current.Y;
                if (current.Y + (float)current.Height > m_bottomMostBorder)
        ...
        public void AddRoom(RoomObj room) {
            m_roomList.Add(room);
            if (room.X < (float)m_leftMostBorder)
                m_leftMostBorder = (int)room.X;
            if (room.X + (float)room.Width > m_rightMostBorder)
                m_rightMostBorder = (int)room.X + room.Width;
            if (room.Y < (float)m_topMostBorder)
                m_topMostBorder = (int)room.Y;
            if (room.Y + (float)room.Height > m_bottomMostBorder)
                    ...
                    if (current != CurrentRoom && current.Bounds.Contains((int)m_player.X, (int)m_player.Y)) {
                        ...
                        if ((CurrentRoom == null || CurrentLevelType != current.LevelType || (CurrentRoom != null && CurrentRoom.Name == "ChallengeBoss")) && current.Name != "Start") {
                            ...
                            if (current.Name != "Boss" && current.Name != "Tutorial" && current.Name != "Ending" && current.Name != "ChallengeBoss") {
                                ...
                                if (m_player.X > (float)current.Bounds.Center.X) {
                                    m_roomTitle.X = 50f;
                                ...
                                Tween.To(m_roomTitle, 0.5f, new Easing(Linear.EaseNone), new[] {
                                    "delay",
                                    "0.2",
                                    "Opacity",
                                    "1"
                                ...
                                m_roomTitle.Opacity = 0f;
                                Tween.To(m_roomEnteringTitle, 0.5f, new Easing(Linear.EaseNone), new[] {
                                    "Opacity",
                                    "1"
                                });
        ...
        public override void Update(GameTime gameTime) {
            ...
            m_projectileIconPool.Update(base.Camera);
            if (!base.IsPaused) {
                ...
                if (!DisableRoomTransitioning && !CollisionMath.Intersects(new Rectangle((int)m_player.X, (int)m_player.Y, 1, 1), base.Camera.Bounds))
        ...
        public void UpdateCamera() {
            if (m_player != null) {
                base.ScreenManager.Camera.X = (float)((int)(m_player.Position.X + GlobalEV.Camera_XOffset));
                base.ScreenManager.Camera.Y = (float)((int)(m_player.Position.Y + GlobalEV.Camera_YOffset));
            }
            if (m_currentRoom != null) {
                if (base.ScreenManager.Camera.Width < m_currentRoom.Width) {
                    if (base.ScreenManager.Camera.Bounds.Left < m_currentRoom.Bounds.Left)
                        base.ScreenManager.Camera.X = (float)((int)((float)m_currentRoom.Bounds.Left + (float)base.ScreenManager.Camera.Width * 0.5f));
                    else if (base.ScreenManager.Camera.Bounds.Right > m_currentRoom.Bounds.Right)
                        base.ScreenManager.Camera.X = (float)((int)((float)m_currentRoom.Bounds.Right - (float)base.ScreenManager.Camera.Width * 0.5f));
                }
                else
                    base.ScreenManager.Camera.X = (float)((int)(m_currentRoom.X + (float)m_currentRoom.Width * 0.5f));
                if (base.ScreenManager.Camera.Height < m_currentRoom.Height) {
                    if (base.ScreenManager.Camera.Bounds.Top < m_currentRoom.Bounds.Top) {
                        base.ScreenManager.Camera.Y = (float)((int)((float)m_currentRoom.Bounds.Top + (float)base.ScreenManager.Camera.Height * 0.5f));
                        return;
                    }
                    if (base.ScreenManager.Camera.Bounds.Bottom > m_currentRoom.Bounds.Bottom) {
                        base.ScreenManager.Camera.Y = (float)((int)((float)m_currentRoom.Bounds.Bottom - (float)base.ScreenManager.Camera.Height * 0.5f));
                        return;
                    }
                }
                else
                    base.ScreenManager.Camera.Y = (float)((int)(m_currentRoom.Y + (float)m_currentRoom.Height * 0.5f));
            }
        }
        ...
        private void UpdateCompass() {
            ...
            if (m_compassDoor != null)
                m_compass.Rotation = CDGMath.AngleBetweenPts(m_player.Position, new Vector2((float)m_compassDoor.Bounds.Center.X, (float)m_compassDoor.Bounds.Center.Y));
        }
        ...
        private void DisplayCompass() {
            ...
            Tween.To(m_compassBG, 0.5f, new Easing(Back.EaseOutLarge), new[] {
                "ScaleX",
                "1",
                "ScaleY",
                "1"
            });
            Tween.To(m_compass, 0.5f, new Easing(Back.EaseOutLarge), new[] {
                "ScaleX",
                "1",
                "ScaleY",
                "1"
        ...
        private void HideCompass() {
            Tween.StopAllContaining(m_compassBG, false);
            Tween.StopAllContaining(m_compass, false);
            Tween.To(m_compassBG, 0.5f, new Easing(Back.EaseInLarge), new[] {
                "ScaleX",
                "0",
                "ScaleY",
                "0"
            });
            Tween.To(m_compass, 0.5f, new Easing(Back.EaseInLarge), new[] {
                "ScaleX",
                "0",
                "ScaleY",
                "0"
        ...
        public void InitializeRenderTargets() {
            ...
            m_fgRenderTarget = new RenderTarget2D(base.Camera.GraphicsDevice, num, num2, false, SurfaceFormat.Bgra5551, DepthFormat.None);
            if (m_shadowRenderTarget != null)
                m_shadowRenderTarget.Dispose();
            m_shadowRenderTarget = new RenderTarget2D(base.Camera.GraphicsDevice, num, num2, false, SurfaceFormat.Bgra4444, DepthFormat.None);
            base.Camera.Begin();
            base.Camera.GraphicsDevice.SetRenderTarget(m_shadowRenderTarget);
            base.Camera.GraphicsDevice.Clear(Color.Black);
            base.Camera.End();
            if (m_lightSourceRenderTarget != null)
                m_lightSourceRenderTarget.Dispose();
            m_lightSourceRenderTarget = new RenderTarget2D(base.Camera.GraphicsDevice, num, num2, false, SurfaceFormat.Bgra4444, DepthFormat.None);
            if (m_finalRenderTarget != null)
                m_finalRenderTarget.Dispose();
            m_finalRenderTarget = new RenderTarget2D(base.Camera.GraphicsDevice, 1320, 720);
            if (m_skyRenderTarget != null)
                m_skyRenderTarget.Dispose();
            m_skyRenderTarget = new RenderTarget2D(base.Camera.GraphicsDevice, num, num2);
            if (m_bgRenderTarget != null)
                m_bgRenderTarget.Dispose();
            m_bgRenderTarget = new RenderTarget2D(base.Camera.GraphicsDevice, 1320, 720, false, SurfaceFormat.Color, DepthFormat.None);
            if (m_traitAuraRenderTarget != null)
                m_traitAuraRenderTarget.Dispose();
            m_traitAuraRenderTarget = new RenderTarget2D(base.Camera.GraphicsDevice, num, num2);
        ...
        public void InitializeBackgroundObjs() {
            ...
            m_foregroundSprite.SetRepeated(true, true, base.Camera, SamplerState.PointWrap);
            m_foregroundSprite.Scale = new Vector2(2f, 2f);
            if (m_backgroundSprite != null)
                m_backgroundSprite.Dispose();
            m_backgroundSprite = new BackgroundObj("CastleBG1_Sprite");
            m_backgroundSprite.SetRepeated(true, true, base.Camera, SamplerState.PointWrap);
            m_backgroundSprite.Scale = new Vector2(2f, 2f);
            ...
            m_backgroundParallaxSprite.SetRepeated(true, true, base.Camera, SamplerState.PointWrap);
            ...
            m_gardenParallaxFG.SetRepeated(true, true, base.Camera, SamplerState.LinearWrap);
        ...
        public void DrawRenderTargets() {
            ...
            if (CurrentRoom != null)
                CurrentRoom.DrawRenderTargets(base.Camera);
            base.Camera.End();
            ...
            base.Camera.GraphicsDevice.SetRenderTarget(m_fgRenderTarget);
            m_foregroundSprite.Draw(base.Camera);
            if (!m_enemiesPaused) {
            ...
            base.Camera.GraphicsDevice.SetRenderTarget(m_traitAuraRenderTarget);
            base.Camera.GraphicsDevice.Clear(Color.Transparent);
            if (CurrentRoom != null) {
                m_traitAura.Position = m_player.Position;
                m_traitAura.Draw(base.Camera);
            }
            base.Camera.GraphicsDevice.SetRenderTarget(m_lightSourceRenderTarget);
            base.Camera.GraphicsDevice.Clear(Color.Transparent);
            if (CurrentRoom != null) {
                m_dungeonLight.Position = m_player.Position;
                m_dungeonLight.Draw(base.Camera);
            }
            base.Camera.End();
            base.Camera.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, SamplerState.PointClamp, null, null);
            m_miniMapDisplay.DrawRenderTargets(base.Camera);
            base.Camera.End();
        ...
        public override void Draw(GameTime gameTime) {
            DrawRenderTargets();
            base.Camera.GraphicsDevice.SetRenderTarget(m_bgRenderTarget);
            base.Camera.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, SamplerState.PointWrap, null, null, null, base.Camera.GetTransformation());
            m_backgroundSprite.Draw(base.Camera);
            if (CurrentRoom != null && base.Camera.Zoom == 1f && (!m_inputMap.Pressed(2) || (m_inputMap.Pressed(2) && (LevelEV.RUN_DEMO_VERSION || LevelEV.CREATE_RETAIL_VERSION))))
                CurrentRoom.DrawBGObjs(base.Camera);
            else {
                foreach (RoomObj current in m_roomList)
                    current.DrawBGObjs(base.Camera);
            }
            base.Camera.End();
            base.Camera.GraphicsDevice.SetRenderTarget(m_finalRenderTarget);
            base.Camera.GraphicsDevice.Clear(Color.Black);
            if (m_enemiesPaused)
            ...
            if (!m_enemiesPaused)
                base.Camera.Draw(m_bgRenderTarget, Vector2.Zero, Color.White);
            base.Camera.End();
            ...
            base.Camera.GraphicsDevice.Textures[1] = m_fgRenderTarget;
            base.Camera.GraphicsDevice.Textures[1].GraphicsDevice.SamplerStates[1] = SamplerState.PointClamp;
            base.Camera.Draw(CurrentRoom.BGRender, base.Camera.TopLeftCorner, Color.White);
            base.Camera.End();
            if (!LevelEV.SHOW_ENEMY_RADII)
                base.Camera.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, SamplerState.PointWrap, null, null, null, base.Camera.GetTransformation());
            else
                base.Camera.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, SamplerState.PointWrap, null, null, null, base.Camera.GetTransformation());
            CurrentRoom.Draw(base.Camera);
            if (LevelEV.SHOW_ENEMY_RADII) {
                foreach (EnemyObj current2 in m_currentRoom.EnemyList)
                    current2.DrawDetectionRadii(base.Camera);
            }
            m_projectileManager.Draw(base.Camera);
            if (m_enemiesPaused) {
                base.Camera.End();
                base.Camera.GraphicsDevice.SetRenderTarget(m_bgRenderTarget);
                base.Camera.GraphicsDevice.Textures[1] = m_traitAuraRenderTarget;
                base.Camera.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, SamplerState.LinearClamp, null, null, Game.InvertShader);
                base.Camera.Draw(m_finalRenderTarget, Vector2.Zero, Color.White);
                base.Camera.End();
            ...
            base.Camera.Draw(Game.GenericTexture, new Rectangle((int)base.Camera.TopLeftCorner.X, (int)base.Camera.TopLeftCorner.Y, 1320, 720), Color.Black * BackBufferOpacity);
            if (!m_player.IsKilled)
                m_player.Draw(base.Camera);
            if (!LevelEV.CREATE_RETAIL_VERSION) {
                DebugTextObj.Position = new Vector2(base.Camera.X, base.Camera.Y - 300f);
                DebugTextObj.Draw(base.Camera);
            }
            m_itemDropManager.Draw(base.Camera);
            m_impactEffectPool.Draw(base.Camera);
            base.Camera.End();
            ...
            m_textManager.Draw(base.Camera);
            if (CurrentRoom.LevelType == GameTypes.LevelType.TOWER)
                m_gardenParallaxFG.Draw(base.Camera);
            m_whiteBG.Draw(base.Camera);
            base.Camera.End();
            if ((CurrentLevelType == GameTypes.LevelType.DUNGEON || Game.PlayerStats.Traits.X == 35f || Game.PlayerStats.Traits.Y == 35f) && (Game.PlayerStats.Class != 13 || (Game.PlayerStats.Class == 13 && !Player.LightOn))) {
                base.Camera.GraphicsDevice.Textures[1] = m_lightSourceRenderTarget;
                base.Camera.GraphicsDevice.Textures[1].GraphicsDevice.SamplerStates[1] = SamplerState.LinearClamp;
                base.Camera.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, SamplerState.LinearClamp, null, null, Game.ShadowEffect);
                if (LevelEV.SAVE_FRAMES)
                    base.Camera.Draw(m_shadowRenderTarget, Vector2.Zero, null, Color.White, 0f, Vector2.Zero, new Vector2(2f, 2f), SpriteEffects.None, 1f);
                else
                    base.Camera.Draw(m_shadowRenderTarget, Vector2.Zero, Color.White);
                base.Camera.End();
            }
            if (CurrentRoom.Name != "Ending") {
                if ((Game.PlayerStats.Traits.X == 3f || Game.PlayerStats.Traits.Y == 3f) && Game.PlayerStats.SpecialItem != 8) {
                    ...
                    Game.GaussianBlur.Draw(m_finalRenderTarget, base.Camera, m_traitAuraRenderTarget);
                }
            ...
            m_projectileIconPool.Draw(base.Camera);
            m_playerHUD.Draw(base.Camera);
            if (m_lastEnemyHit != null && m_enemyHUDCounter > 0f)
                m_enemyHUD.Draw(base.Camera);
            ...
            if (CurrentRoom.Name != "Start" && CurrentRoom.Name != "Boss" && CurrentRoom.Name != "ChallengeBoss" && m_miniMapDisplay.Visible) {
                m_mapBG.Draw(base.Camera);
                m_miniMapDisplay.Draw(base.Camera);
            }
            if (CurrentRoom.Name != "Boss" && CurrentRoom.Name != "Ending") {
                m_compassBG.Draw(base.Camera);
                m_compass.Draw(base.Camera);
            }
            m_objectivePlate.Draw(base.Camera);
            m_roomEnteringTitle.Draw(base.Camera);
            m_roomTitle.Draw(base.Camera);
            ...
            base.Camera.End();
            base.Camera.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, SamplerState.PointClamp, null, null);
            m_blackBorder1.Draw(base.Camera);
            m_blackBorder2.Draw(base.Camera);
            base.Camera.End();
            base.Camera.GraphicsDevice.SetRenderTarget(m_bgRenderTarget);
            Game.RippleEffect.Parameters["width"].SetValue(ShoutMagnitude);
            Vector2 vector = m_player.Position - base.Camera.TopLeftCorner;
            ...
            if (CurrentRoom.Name != "Ending") {
                if ((Game.PlayerStats.Traits.X == 1f || Game.PlayerStats.Traits.Y == 1f) && Game.PlayerStats.SpecialItem != 8) {
                    ...
                    base.Camera.Draw(m_finalRenderTarget, Vector2.Zero, color);
                    m_creditsText.Draw(base.Camera);
                    m_creditsTitleText.Draw(base.Camera);
                    base.Camera.End();
                    base.Camera.GraphicsDevice.SetRenderTarget((base.ScreenManager as RCScreenManager).RenderTarget);
                    base.Camera.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, SamplerState.LinearClamp, null, null, null);
                }
        ...
        public void RunWhiteSlashEffect() {
            ...
            m_whiteBG.Scale = new Vector2((float)(CurrentRoom.Width / m_whiteBG.Width), (float)(m_currentRoom.Height / m_whiteBG.Height));
            m_whiteBG.Opacity = 1f;
            Tween.To(m_whiteBG, 0.2f, new Easing(Tween.EaseNone), new[] {
                "Opacity",
                "0"
            });
        ...
        public void LightningEffectOnce() {
            ...
            m_whiteBG.Scale = new Vector2((float)(CurrentRoom.Width / m_whiteBG.Width), (float)(m_currentRoom.Height / m_whiteBG.Height));
            m_whiteBG.Opacity = 1f;
            Tween.To(m_whiteBG, 1f, new Easing(Tween.EaseNone), new[] {
                "Opacity",
                "0"
            });
        ...
        public void RunCinematicBorders(float duration) {
            ...
            m_blackBorder1.Y = 0f;
            m_blackBorder2.Y = (float)(720 - m_borderSize);
            float num = 1f;
            Tween.By(m_blackBorder1, num, new Easing(Quad.EaseInOut), new[] {
                "delay",
                (duration - num).ToString(),
                "Y",
                (-m_borderSize).ToString()
            });
            Tween.By(m_blackBorder2, num, new Easing(Quad.EaseInOut), new[] {
                "delay",
                (duration - num).ToString(),
                "Y",
                m_borderSize.ToString()
            });
            Tween.To(m_blackBorder1, num, new Easing(Linear.EaseNone), new[] {
                "delay",
                (duration - num + 0.2f).ToString(),
                "Opacity",
                "0"
            });
            Tween.To(m_blackBorder2, num, new Easing(Linear.EaseNone), new[] {
                "delay",
                (duration - num + 0.2f).ToString(),
                "Opacity",
                "0"
        ...
        public void DisplayMap(bool isTeleporterScreen) {
            (base.ScreenManager as RCScreenManager).AddRoomsToMap(m_miniMapDisplay.AddedRoomsList);
        ...
        public void CastTimeStop(float duration) {
            ...
            PauseAllEnemies();
            Tween.To(m_traitAura, 0.2f, new Easing(Tween.EaseNone), new[] {
                "ScaleX",
                "100",
                "ScaleY",
                "100"
        ...
        public void RefreshMapChestIcons() {
            ...
            (base.ScreenManager as RCScreenManager).RefreshMapScreenChestIcons(CurrentRoom);
        }
        ...
        public void DisplayObjective(string objectiveTitle, string objectiveDescription, string objectiveProgress, bool tween) {
            ...
            if (tween) {
                m_objectivePlateTween = Tween.By(m_objectivePlate, 0.5f, new Easing(Back.EaseOut), new[] {
                    "X",
                    "-300"
                });
                return;
        ...
        public void ResetObjectivePlate(bool tween) {
            if (m_objectivePlate != null) {
                ...
                if (m_objectivePlateTween != null && m_objectivePlateTween.TweenedObject == m_objectivePlate && m_objectivePlateTween.Active)
                    m_objectivePlateTween.StopTween(false);
                if (tween) {
                    Tween.By(m_objectivePlate, 0.5f, new Easing(Back.EaseIn), new[] {
                        "X",
                        "300"
                    });
                    return;
        ...
        public override void OnEnter() {
            ...
            if (Game.PlayerStats.Class == 13) {
                ...
                (base.ScreenManager as RCScreenManager).AddIconsToMap(RoomList);
            }
            if (Game.PlayerStats.EyeballBossBeaten)
        ...
        public void RevealMorning() {
            ...
            Tween.To(m_sky, 2f, new Easing(Tween.EaseNone), new[] {
                "MorningOpacity",
                "1"
            });
        }
        ...
        public void ZoomOutAllObjects() {
            Vector2 vector = new Vector2((float)CurrentRoom.Bounds.Center.X, (float)CurrentRoom.Bounds.Center.Y);
            ...
            foreach (GameObj current in CurrentRoom.GameObjList) {
                ...
                if (Math.Abs(num3) > Math.Abs(num2)) {
                    ...
                    Tween.By(current, 0.5f, new Easing(Back.EaseIn), new[] {
                        "delay",
                        num.ToString(),
                        "Y",
                        num2.ToString()
        ...
        public void ZoomInAllObjects(List<Vector2> objPositions) {
            ...
            float num2 = 1f;
            foreach (GameObj current in CurrentRoom.GameObjList) {
                Tween.By(current, 0.5f, new Easing(Back.EaseOut), new[] {
                    "delay",
                    num2.ToString(),
                    "X",
