...
namespace RogueCastle {
    public class StartingRoomObj : RoomObj {
        ...
        public StartingRoomObj() {
            ...
            m_blacksmith.Position = new Vector2(700f, 660f - ((float)m_blacksmith.Bounds.Bottom - m_blacksmith.Y) - 1f);
            ...
            m_blacksmithBoard.Position = new Vector2(m_blacksmith.X - (float)(m_blacksmithBoard.Width / 2) - 35f, (m_blacksmith.Bounds.Bottom - m_blacksmithBoard.Height - 1));
            ...
            m_enchantress.Position = new Vector2(1150f, 660f - ((float)m_enchantress.Bounds.Bottom - m_enchantress.AnchorY) - 2f);
            ...
            m_tent.Position = new Vector2(m_enchantress.X - (float)(m_tent.Width / 2) + 5f, (float)(m_enchantress.Bounds.Bottom - m_tent.Height));
            ...
            m_architect.Position = new Vector2(1550f, 660f - ((float)m_architect.Bounds.Bottom - m_architect.AnchorY) - 2f);
            ...
            m_tollCollector.Position = new Vector2(2565f, 420f - ((float)m_tollCollector.Bounds.Bottom - m_tollCollector.AnchorY));
        ...
        public override void Initialize() {
            ...
            for (int i = 0; i < base.GameObjList.Count; i++) {
                if (base.GameObjList[i].Name == "Mountains 1")
                    m_mountain1 = (base.GameObjList[i] as SpriteObj);
                if (base.GameObjList[i].Name == "Mountains 2")
                    m_mountain2 = (base.GameObjList[i] as SpriteObj);
            }
            base.Initialize();
        }
        ...
        public override void OnEnter() {
            ...
            if (m_blacksmithAnvilSound == null) {
                m_blacksmithAnvilSound = new FrameSoundObj(m_blacksmith.GetChildAt(5) as IAnimateableObj, Player, 7, new[] {
                    "Anvil1",
                    "Anvil2",
                    "Anvil3"
                });
            ...
            Player.Position = new Vector2(0f, 660f - ((float)Player.Bounds.Bottom - Player.Y));
            Player.State = 1;
        ...
        public override void Update(GameTime gameTime) {
            ...
            if (!m_playerWalkedOut) {
                if (!Player.ControlsLocked && Player.X < (float)this.Bounds.Left) {
                ...
                else if (!Player.ControlsLocked && Player.X > (float)this.Bounds.Right && !TollCollectorAvailable) {
                    m_playerWalkedOut = true;
                    LoadLevel();
                }
            }
            ...
            m_blacksmithIcon.Visible = false;
            if (Player != null && CollisionMath.Intersects(Player.TerrainBounds, m_blacksmith.Bounds) && Player.IsTouchingGround && SmithyAvailable)
                m_blacksmithIcon.Visible = true;
            m_blacksmithIcon.Position = new Vector2(m_blacksmithIconPosition.X, m_blacksmithIconPosition.Y - 70f + (float)Math.Sin((totalGameTime * 20f)) * 2f);
            m_enchantressIcon.Visible = false;
            Rectangle b = new Rectangle((int)(m_enchantress.X - 100f), (int)m_enchantress.Y, m_enchantress.Bounds.Width + 100, m_enchantress.Bounds.Height);
            if (Player != null && CollisionMath.Intersects(Player.TerrainBounds, b) && Player.IsTouchingGround && EnchantressAvailable)
                m_enchantressIcon.Visible = true;
            m_enchantressIcon.Position = new Vector2(m_enchantressIconPosition.X + 20f, m_enchantressIconPosition.Y + (float)Math.Sin((totalGameTime * 20f)) * 2f);
            if (Player != null && CollisionMath.Intersects(Player.TerrainBounds, new Rectangle((int)m_architect.X - 100, (int)m_architect.Y, m_architect.Width + 200, m_architect.Height)) && Player.X < m_architect.X && Player.Flip == SpriteEffects.None && ArchitectAvailable)
                m_architectIcon.Visible = true;
            else
                m_architectIcon.Visible = false;
            m_architectIcon.Position = new Vector2(m_architectIconPosition.X, m_architectIconPosition.Y + (float)Math.Sin((totalGameTime * 20f)) * 2f);
            if (Player != null && CollisionMath.Intersects(Player.TerrainBounds, new Rectangle((int)m_tollCollector.X - 100, (int)m_tollCollector.Y, m_tollCollector.Width + 200, m_tollCollector.Height)) && Player.X < m_tollCollector.X && Player.Flip == SpriteEffects.None && TollCollectorAvailable && m_tollCollector.SpriteName == "NPCTollCollectorIdle_Character")
                m_tollCollectorIcon.Visible = true;
            else
                m_tollCollectorIcon.Visible = false;
            m_tollCollectorIcon.Position = new Vector2(m_tollCollector.X - (float)(m_tollCollector.Width / 2) - 10f, m_tollCollector.Y - (float)m_tollCollectorIcon.Height - (m_tollCollector.Height / 2) + (float)Math.Sin((totalGameTime * 20f)) * 2f);
            m_blacksmithNewIcon.Visible = false;
            if (SmithyAvailable) {
            ...
            if (Player.Bounds.Right > m_tollCollector.Bounds.Left && TollCollectorAvailable) {
                Player.X = (float)m_tollCollector.Bounds.Left - ((float)Player.Bounds.Right - Player.X);
        ...
        private void HandleInput() {
            if (!m_controlsLocked) {
                if (Player.State != 4) {
                    if (m_blacksmithIcon.Visible && (Game.GlobalInput.JustPressed(16) || Game.GlobalInput.JustPressed(17)))
                        MovePlayerTo(m_blacksmith);
                    if (m_enchantressIcon.Visible && (Game.GlobalInput.JustPressed(16) || Game.GlobalInput.JustPressed(17)))
                        MovePlayerTo(m_enchantress);
                    if (m_architectIcon.Visible && (Game.GlobalInput.JustPressed(16) || Game.GlobalInput.JustPressed(17))) {
                ...
                if (m_tollCollectorIcon.Visible && (Game.GlobalInput.JustPressed(16) || Game.GlobalInput.JustPressed(17))) {
                    ...
                    if (!Game.PlayerStats.SpokeToTollCollector)
                        ...
                        rCScreenManager2.DialogueScreen.SetDialogue("Meet Toll Collector Skip" + (int)Math.Round(num, MidpointRounding.AwayFromZero));
                    }
                    RunTollPaidSelection(rCScreenManager2);
                }
            }
        ...
        public void MovePlayerTo(GameObj target) {
            ...
            if (Player.X != target.X - 150f) {
                ...
                float num = CDGMath.DistanceBetweenPts(Player.Position, new Vector2(target.X - 150f, target.Y)) / Player.Speed;
                Player.UpdateCollisionBoxes();
                Player.State = 1;
                Player.IsWeighted = false;
                ...
                Player.RunExternalLogicSet(logicSet);
                Tween.To(Player, num, new Easing(Tween.EaseNone), new[] {
                    "X",
            ...
            if (target != m_blacksmith) {
                if (target == m_enchantress) {
                    if (!Game.PlayerStats.SpokeToEnchantress) {
                        ...
                        DialogueScreen arg_1A1_0 = (Player.AttachedLevel.ScreenManager as RCScreenManager).DialogueScreen;
        ...
        public void TollPaid(bool chargeFee) {
            if (chargeFee) {
                ...
                if (num > 0f)
                    Player.AttachedLevel.TextManager.DisplayNumberStringText(-(int)num, "gold", Color.Yellow, new Vector2(Player.X, Player.Bounds.Top));
            }
        ...
        public void ActivateArchitect() {
            ...
            Tween.By(m_architectBlock, 0.8f, new Easing(Tween.EaseNone), new[] {
                "delay",
                "1.1",
                "Y",
                "135"
            ...
            Tween.RunFunction(1.5f, Player.AttachedLevel.ImpactEffectPool, "SkillTreeDustEffect", new object[] {
                new Vector2(m_screw.X - (float)m_screw.Width / 2f, m_screw.Y - 40f),
                true,
                m_screw.Width
            });
        ...
        public void UpdateShake() {
            if (m_horizontalShake)
                Player.AttachedLevel.Camera.X = m_shakeStartingPos.X + (float)CDGMath.RandomPlusMinus() * (CDGMath.RandomFloat(0f, 1f) * m_screenShakeMagnitude);
            if (m_verticalShake)
                Player.AttachedLevel.Camera.Y = m_shakeStartingPos.Y + (float)CDGMath.RandomPlusMinus() * (CDGMath.RandomFloat(0f, 1f) * m_screenShakeMagnitude);
        }
